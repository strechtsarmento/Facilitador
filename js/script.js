...
let cameraListenersConfigurados = false;
      ...
      <div class="modulo-alinhamento">
        ...
        <div id="container-camera-nativa" class="hidden">
          ...
          <div id="controles-overlay-live" class="painel-inferior-ajustes-cam hidden">
            <div class="guia-overlay-titulo">👻 Foto anterior usada como guia</div>
            <div class="linha-controle-slider">
              <span>Transparência:</span>
              <input type="range" id="opacidade-live-range" min="5" max="95" value="45" oninput="atualizarOpacidadeLive(this.value)" />
              <strong id="opacidade-live-valor">45%</strong>
            </div>
            <div class="grupo-botoes grupo-ajuste-overlay">
              <button class="btn-mini" onclick="ajustarZoomOverlay(-0.1)">➖ Menor</button>
              <button class="btn-mini" onclick="ajustarZoomOverlay(0.1)">➕ Maior</button>
              <button class="btn-mini btn-reset" onclick="resetarAjustesAntesOverlay()">🔄 Centralizar</button>
            </div>
            <p class="dica-overlay">Arraste a imagem fantasma com um dedo e use dois dedos para ajustar o tamanho.</p>
          </div>
...
async function confirmarFotoAntes() {
  if (!fotoAntesData) return;

  // Prepara a referência antes de liberar a câmera, evitando o overlay vazio.
  imgAntesObjeto = new Image();
  imgAntesObjeto.src = fotoAntesData;
  try {
    await imgAntesObjeto.decode();
  } catch (_) {
    await new Promise((resolve) => {
      imgAntesObjeto.onload = resolve;
      imgAntesObjeto.onerror = resolve;
    });
  }

  salvarNaGaleriaLocal(fotoAntesData);
  localStorage.setItem("ultima_foto_antes", fotoAntesData);
...
async function abrirPainelCamera(alvo) {
  alvoAtualCamera = alvo;

  if (alvo === "depois" && fotoAntesData && (!imgAntesObjeto || !imgAntesObjeto.complete)) {
    imgAntesObjeto = new Image();
    imgAntesObjeto.src = fotoAntesData;
    try { await imgAntesObjeto.decode(); } catch (_) {}
  }
...
async function inicializarStreamCamera() {
  ...
  try {
    ...
    video.onloadedmetadata = () => {
      // O canvas acompanha a resolução real da câmera. O desenho usa "cover"
      // para manter a mesma proporção 3:4 exibida na tela, sem deformar.
      canvasVisor.width = video.videoWidth || 720;
      canvasVisor.height = Math.round(canvasVisor.width * 4 / 3);
    ...
    btnDisparar.onclick = () => {
      const capturarFotoFinal = () => {
        ...
        desenharCover(ctxSnap, video, canvasSnapshot.width, canvasSnapshot.height, modoCamera === "user");
...
function configurarArrastoOverlayCamera(canvasElement) {
  if (cameraListenersConfigurados) return;
  cameraListenersConfigurados = true;
...
function desenharCover(ctx, origem, larguraDestino, alturaDestino, espelhar = false) {
  const origemW = origem.videoWidth || origem.naturalWidth || origem.width;
  const origemH = origem.videoHeight || origem.naturalHeight || origem.height;
  if (!origemW || !origemH) return;

  const escala = Math.max(larguraDestino / origemW, alturaDestino / origemH);
  const sw = larguraDestino / escala;
  const sh = alturaDestino / escala;
  const sx = (origemW - sw) / 2;
  const sy = (origemH - sh) / 2;

  ctx.save();
  if (espelhar) {
    ctx.translate(larguraDestino, 0);
    ctx.scale(-1, 1);
  }
  ctx.drawImage(origem, sx, sy, sw, sh, 0, 0, larguraDestino, alturaDestino);
  ctx.restore();
}
...
function loopRenderVisor(video, canvas, ctx) {
  ...
  desenharCover(ctx, video, canvas.width, canvas.height, modoCamera === "user");
...
function ajustarZoomOverlay(fator) {
  transformacoesAntesOverlay.zoom = Math.max(0.3, Math.min(transformacoesAntesOverlay.zoom + fator, 5));
}
function atualizarOpacidadeLive(valor) {
  opacidadeLiveGlobal = valor / 100;
  const indicador = document.getElementById("opacidade-live-valor");
  if (indicador) indicador.textContent = `${valor}%`;
}
